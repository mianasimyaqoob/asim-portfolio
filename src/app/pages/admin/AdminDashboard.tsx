  ...
  return (
    <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
      <button
        type="button"
        onClick={onToggle}
        className="w-full flex items-center justify-between px-5 py-4 bg-muted/50/60 hover:bg-muted/50 transition-colors text-left"
      >
...
function FieldLabel({ children, hint }: { children: React.ReactNode; hint?: string }) {
  return (
    <div className="mb-1.5">
      <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-wide">{children}</label>
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-card rounded-2xl shadow-2xl p-6 max-w-sm w-full"
      >
        ...
        <p className="text-sm text-muted-foreground text-center mb-6">
          "<span className="font-semibold text-foreground">{project.title}</span>" will be permanently removed from your portfolio.
        </p>
    ...
    >
      ...
      <div className="shrink-0 bg-card border-b border-border/50 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button type="button" onClick={onCancel} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-gray-900 transition-colors font-medium">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  ...
                  <Input
                    ...
                    className="bg-muted/50 border-border/50" placeholder="e.g. Social Vibing" />
                </div>
                <div className="sm:col-span-2">
                  ...
                  <Input value={formData.slug} onChange={(e) => set({ slug: e.target.value })}
                    className="bg-muted/50 border-border/50 font-mono text-sm" placeholder="social-vibing" />
                </div>
                <div className="sm:col-span-2">
                  ...
                  <Input value={formData.tagline} onChange={(e) => set({ tagline: e.target.value })}
                    className="bg-muted/50 border-border/50" placeholder="A social platform for connecting through vibe-based matching" />
                </div>
                <div>
                    ...
                    {CATEGORIES.map(c => {
                      ...
                      return (
                        <button
                          ...
                          className={`text-[10px] px-2.5 py-1 rounded-full border font-bold transition-all ${active ? 'bg-indigo-500 border-indigo-500 text-white' : 'bg-card border-border/50 text-muted-foreground hover:border-indigo-300'}`}
                        >
                          {c}
                        </button>
                      );
                ...
                <div>
                  ...
                  <div className="flex gap-2 mt-1">
                    <button
                      ...
                      className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-xs font-bold transition-all flex-1 justify-center ${formData.status === 'published' ? 'bg-green-100 border-green-200 text-green-700' : 'bg-muted border-border/50 text-muted-foreground'}`}
                    >
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-xs font-bold transition-all flex-1 justify-center ${formData.featured ? 'bg-amber-100 border-amber-200 text-amber-700' : 'bg-muted border-border/50 text-muted-foreground'}`}
                    >
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full h-10 rounded-md border border-border/50 bg-muted/50 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                  >
                ...
                <div>
                  ...
                  <Input
                    ...
                    className="bg-muted/50 border-border/50" placeholder="e.g. Remote (Upwork)"
                  />
                </div>
              </div>
            </SectionCard>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Textarea
                    ...
                    className="bg-muted/50 border-border/50 min-h-[80px] text-sm"
                ...
                <div>
                    ...
                    {TECH_PRESETS.map(t => {
                      ...
                      return (
                        <button
                          ...
                          className={`text-[11px] px-2.5 py-1 rounded-full border font-medium transition-all ${active ? 'bg-blue-500 border-blue-500 text-white' : 'bg-card border-border/50 text-muted-foreground hover:border-blue-300'}`}
                        >{t}</button>
                      );
                    })}
                  </div>
                  ...
                  <div className="flex gap-2">
                    <Input value={techInput} onChange={(e) => setTechInput(e.target.value)}
                      ...
                      className="bg-muted/50 border-border/50 text-sm" placeholder="Add custom tech…" />
                    ...
                    {formData.extraImages && formData.extraImages.length > 0 && (
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-2">
                        {formData.extraImages.map((img, idx) => (
                          <div key={idx} className="group relative aspect-video rounded-lg overflow-hidden border border-border/50 bg-muted/50">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <Input value={formData.liveUrl} onChange={(e) => set({ liveUrl: e.target.value })}
                      className="bg-muted/50 border-border/50 pl-8 text-sm" placeholder="https://…" />
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <Input value={formData.githubUrl} onChange={(e) => set({ githubUrl: e.target.value })}
                      className="bg-muted/50 border-border/50 pl-8 text-sm" placeholder="https://github.com/…" />
                  </div>
                </div>
                <div className="sm:col-span-2">
          ...
          <div className="space-y-4">
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <Textarea value={formData.overview} onChange={(e) => set({ overview: e.target.value })}
                    className="bg-muted/50 border-border/50 min-h-[90px] text-sm"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input value={formData.role} onChange={(e) => set({ role: e.target.value })}
                      className="bg-muted/50 border-border/50 text-sm" placeholder="e.g. Full-Stack Developer" />
                  </div>
                  <div>
                    ...
                    <Input value={formData.timeline} onChange={(e) => set({ timeline: e.target.value })}
                      className="bg-muted/50 border-border/50 text-sm" placeholder="e.g. Nov–Dec 2024 · 2 months" />
                  </div>
                </div>
                <div>
            ...
            >
              <div className="space-y-4">
                ...
                <div className="flex bg-muted p-1 rounded-lg w-fit gap-1">
                  {(['upload', 'url'] as const).map(src => (
                    <button key={src} type="button" onClick={() => setImageSource(src)}
                      className={`px-4 py-1.5 text-xs font-semibold rounded-md transition-all capitalize ${imageSource === src ? 'bg-card text-blue-600 shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
                ...
                {imageSource === 'url' ? (
                  <div>
                    ...
                    <Input value={formData.image} onChange={(e) => set({ image: e.target.value })}
                      className="bg-muted/50 border-border/50 text-sm" placeholder="https://…" />
                  </div>
                ) : (
                  <div>
                    ...
                    {!formData.image ? (
                      <div
                        ...
                        className="aspect-video rounded-xl border-2 border-dashed border-border/50 bg-muted/50 hover:border-blue-400 hover:bg-blue-50/30 cursor-pointer flex flex-col items-center justify-center gap-2 transition-all"
                      >
                        {isUploading ? (
                          ...
                          <>
                            ...
                            <div className="text-center">
                              ...
                              <p className="text-xs text-muted-foreground mt-0.5">PNG, JPG, WebP · max 3 MB · 1600×900 recommended</p>
                            </div>
                          </>
                        )}
                      </div>
                ...
                {formData.image && (
                  <div className="space-y-3">
                    <div className="relative aspect-video w-full rounded-xl overflow-hidden border border-border/50 bg-muted group">
                    ...
                    {/* Image settings */}
                    <div className="grid grid-cols-2 gap-3 p-3 bg-muted/50 rounded-xl border border-border text-xs">
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold text-muted-foreground uppercase flex items-center gap-1"><Settings className="w-3 h-3" /> Object Fit</label>
                        <select value={formData.imageConfig?.objectFit}
                          ...
                          className="w-full text-xs bg-card border border-border/50 rounded-md py-1 px-2 focus:ring-2 focus:ring-blue-500/20 focus:outline-none">
                          <option value="cover">Cover (fill)</option>
                          <option value="contain">Contain (fit)</option>
                        </select>
                      </div>
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold text-muted-foreground uppercase">Corner Radius: {formData.imageConfig?.cornerRadius}px</label>
                      ...
                      <div className="col-span-2 space-y-1">
                        <label className="text-[10px] font-bold text-muted-foreground uppercase">Background Fill</label>
                        <div className="flex items-center gap-2">
                          <input type="color" value={formData.imageConfig?.background}
                            ...
                            className="w-8 h-8 rounded cursor-pointer border border-border/50 p-0.5 bg-card" />
                          <span className="font-mono text-[11px] text-muted-foreground">{formData.imageConfig?.background}</span>
  ...
  return (
    <div className="min-h-screen bg-muted flex">
      ...
      <motion.aside
        ...
        className="fixed left-0 top-0 bottom-0 w-64 bg-card border-r border-border/50 z-50 lg:translate-x-0 lg:static lg:flex-shrink-0 flex flex-col"
      >
          ...
          {navItems.map(({ id, icon: Icon, label }) => (
            <button key={id} onClick={() => { setActiveTab(id); setSidebarOpen(false); }}
              ...
              {id === 'projects' && projects.length > 0 && (
                <span className={`ml-auto text-[10px] font-bold px-2 py-0.5 rounded-full ${activeTab === id ? 'bg-card/20 text-white' : 'bg-muted text-muted-foreground'}`}>{projects.length}</span>
              )}
            </button>
          ))}
        </nav>
          ...
          <div className="px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Manage your portfolio {activeTab}</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
            ...
            <div className="space-y-5">
              ...
              {isLoading ? (
                ...
                <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
                  {/* Table header */}
                  <div className="hidden md:grid grid-cols-[2fr_2fr_1fr_1fr_1fr_1fr_auto] gap-4 px-5 py-3 bg-muted/50 border-b border-border">
                    {['Project', 'Tagline', 'Category', 'Status', 'Featured', 'Updated', ''].map((h, i) => (
                      <div key={i} className={`text-[10px] font-bold text-muted-foreground uppercase tracking-wider ${i === 6 ? 'text-right' : ''}`}>{h}</div>
                    ))}
                  </div>
                  {/* Rows */}
                  <div className="divide-y divide-gray-50">
                    {projects.map((project, idx) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-lg overflow-hidden border border-border bg-muted/50 shrink-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="text-[10px] text-muted-foreground font-mono truncate">{project.slug}</div>
                          </div>
                        </div>
                        {/* Tagline */}
                        <div className="text-sm text-muted-foreground line-clamp-1 hidden md:block">{project.tagline || <span className="italic text-gray-300">No tagline</span>}</div>
                        {/* Category */}
                        ...
                        <div className="hidden md:block">
                          <span className={`inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-1 rounded-full ${project.status === 'published' ? 'bg-green-100 text-green-700' : 'bg-muted text-muted-foreground'}`}>
