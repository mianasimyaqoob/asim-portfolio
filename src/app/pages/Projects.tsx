  ...
  return (
    <>
      <div className="min-h-screen pt-20">
        ...
        {/* Category Filters */}
        <section className="py-8 border-b border-border/50/50">
          <div className="max-w-7xl mx-auto px-6">
            <motion.div
              initial="hidden"
              animate="visible"
          ...
          <div className="max-w-7xl mx-auto px-6">
            <AnimatePresence mode="wait">
              {isLoading ? (
                ...
                >
                  <p className="text-xl text-muted-foreground">No projects in this category yet.</p>
                </motion.div>
              ) : (
                <motion.div
                ...
                >
                  {filteredProjects.map((project) => (
                    <motion.div
                      ...
                      className="bg-card rounded-2xl overflow-hidden border border-border cursor-pointer group flex flex-col h-full hover:shadow-xl transition-all duration-300"
                      ...
                      <div className="relative h-44 overflow-hidden">
                        ...
                        {project.projectStatus === 'ongoing' && (
                          <div className="absolute top-3 right-3 px-2 py-0.5 bg-green-500 text-white text-[10px] font-bold rounded-full flex items-center gap-1">
                            <div className="w-1 h-1 bg-card rounded-full animate-pulse" />
                            Live
                          </div>
                        )}
                      </div>
                      ...
                      <div className="p-5 flex flex-col flex-1">
                        ...
                        <div className="flex flex-wrap gap-1.5 mb-4 mt-auto">
                          {project.tech.slice(0, 3).map((tech) => (
                            <span key={tech} className="px-2 py-0.5 bg-muted/50 text-muted-foreground text-[10px] border border-border rounded-md">
                              {tech}
                            </span>
                          ))}
                          {project.tech.length > 3 && (
                            <span className="text-[10px] text-muted-foreground font-medium">+{project.tech.length - 3}</span>
                          )}
                        </div>
                ...
                >
                  ...
                  <DialogHeader>
                    <div className="space-y-4">
                      ...
                      {/* Overview */}
                      <p className="text-lg text-muted-foreground leading-relaxed border-l-4 border-blue-500 pl-4 bg-blue-50/50 py-3 rounded-r-lg">
                  ...
                  {/* Hero Image & Gallery */}
                  <div className="space-y-4">
                    <div className="relative h-96 rounded-2xl overflow-hidden group bg-muted/50 border border-border shadow-inner">
                      <AnimatePresence mode="wait">
                        <motion.div
                      ...
                      {allSelectedProjectImages.length > 1 && (
                        <>
                          <button
                            ...
                            className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-card/80 backdrop-blur-md rounded-full flex items-center justify-center text-foreground opacity-0 group-hover:opacity-100 transition-opacity shadow-lg"
                          >
                            <ChevronLeft className="w-6 h-6" />
                          </button>
                          <button
                            onClick={() => setActiveImageIndex((prev) => (prev === allSelectedProjectImages.length - 1 ? 0 : prev + 1))}
                            className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-card/80 backdrop-blur-md rounded-full flex items-center justify-center text-foreground opacity-0 group-hover:opacity-100 transition-opacity shadow-lg"
                          >
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="flex gap-4 pt-4">
                      ...
                      {!selectedProject.liveUrl && !selectedProject.githubUrl && (
                        <p className="text-sm text-muted-foreground italic flex-1 text-center py-4">
